...
func setup(table_size: Vector2) -> void:
	...
	var placeholder_width: float = card_width + (MAX_CARDS - 1) * card_spacing
...
func get_card_position(index: int, card_count: int) -> Vector2:
	var card_spacing: float = card_size.x * (1.0 - CARD_OVERLAP)
	var total_width: float = card_size.x + (card_count - 1) * card_spacing
	# Center the entire group inside PlayedCards
	var start_x: float = (size.x - total_width) / 2.0
	var x: float = start_x + card_size.x / 2.0 + index * card_spacing
...
func get_next_card_position() -> Vector2:
	return get_card_position(cards.size(), cards.size() + 1)
...
func layout_cards() -> void:
	...
	for i in range(card_count):
		...
		card.position = get_card_position(i, card_count)
		card.z_index = i
...
func receive_cards(cards_to_play: Array[Card]) -> void:
	...
	for card in cards_to_play:
		...
		var target_position: Vector2 = get_next_card_global_position()
		var target_scale: float = get_card_scale()
